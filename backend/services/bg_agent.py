...
async def generate_reply(tool: str, result: dict, context: list = []) -> str | None:
    ...
    print("📡 [LLM TRY 1/3] Gemini failed/unconfigured. Falling back to Groq API (Llama-3.1)...")
    ...
    print("📡 [LLM TRY 2/3] Directing request to Gemini API...")
    ...
    print(
        f"📡 [LLM TRY 3/3] Groq failed/unconfigured. Falling back to local Ollama runner ({settings.OLLAMA_MODEL})..."
    )
    ...
    print(
        "❌ [LLM FAILURE] All active LLM providers (Gemini, Groq, Ollama) failed to respond or are unconfigured. Falling back to local hardcoded templates."
    )
    return None
