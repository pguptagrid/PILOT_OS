...
from backend.core.events import startup_pipeline, shutdown_pipeline
...
async def route_auth_middleware(request: Request, call_next):
    ...
    if path.startswith("/api/v1/"):
        ...
        is_public = any(p in path for p in ["/auth/signup", "/auth/login", "/auth/send-otp", "/auth/verify-otp", "/auth/forgot-password", "/auth/verify-forgot-otp", "/ppt/viewer", "/ppt/upload_stream", "/ppt/upload_instant", "/ppt/render_stream", "/compile-notes"])
        if not is_public:
