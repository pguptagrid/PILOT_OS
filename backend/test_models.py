...
async def test_stt_whisper():
    ...
    print(f" - Transcription result: {result_text!r} (expected to be blank/filtered due to no speech harmonics)")
...
async def test_background_agent_gemini():
    ...
    mock_result = {
        "status": "ok",
        "flights": [
            {"airline": "Delta", "price": "$380", "dep": "10:15 AM", "arr": "1:45 PM"}
        ]
    }
    
    logger.info(f"Requesting background tool summarization reply from Gemini API...")
    t0 = time.time()
