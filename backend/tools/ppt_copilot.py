...
from backend.core.slide_store import _slide_store, _current_slide, resolve_sid
...
async def ppt_delete_slide(args: dict, session_id: str) -> dict:
    from backend.api.ppt import _slide_store, _current_slide, save_and_render_pptx, resolve_sid
    from backend.queues.bus import bus
...
async def ppt_create_slides(args: dict, session_id: str) -> dict:
    ...
    topic = query
    topic = re.sub(r"\b(?:with|having|of|containing)?\s*\d+\s*(?:slide|page)s?\b", "", topic, flags=re.IGNORECASE)
    topic = re.sub(r"\b(?:with|having|of|containing)?\s*(?:" + "|".join(word_to_num.keys()) + r")\s*(?:slide|page)s?\b", "", topic, flags=re.IGNORECASE)
    ...
    slides = await create_presentation_from_prompt(prompt=topic, session_id=session_id, slide_count=slide_count)
...
async def ppt_qa(args: dict, session_id: str) -> dict:
    ...
    if slide_index is None:
        current_keywords = ["current slide", "this slide", "active slide", "present slide", "current page", "this page"]
...
async def ppt_improvise_slide(args: dict, session_id: str) -> dict:
    from backend.api.ppt import save_and_render_pptx, improvise_slide_content
    from backend.queues.bus import bus
    from backend.core.slide_store import resolve_sid
    ...
    try:
        ...
        return {"status": "error", "spoken_reply": "I encountered an error trying to improvise the slide. Please try again."}